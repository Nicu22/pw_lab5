...
# Checks if the cached data for a webpage is older than 5 seconds.
def cache_expired(timestamp):
    return (datetime.now() - datetime.fromisoformat(timestamp)).total_seconds() > 5
...
def search_with_bing(terms):
    ...
    if content:
        ...
        results = soup.find(id="b_results").find_all("li", class_="b_algo")
...
# Initializes the program, parsing command-line arguments.
